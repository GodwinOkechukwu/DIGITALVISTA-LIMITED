...
This guide explains how to build and run the digitalvista  Innovations application using Docker.
...
```bash
docker build -t digitalvista:latest .
```

Or with a specific version tag:

```bash
docker build -t digitalvista:1.0.0 .
```
...
```bash
docker run -d \
  --name digitalvista_app \
  -p 4019:4019 \
  --env-file .env.local \
  digitalvista:latest
```

Replace `digitalvista:latest` with your image name/tag.
...
| `TABLE_PREFIX`         | Database table prefix                | No       | `digitalvista_`            |
...
docker exec -it digitalvista_postgres psql -U postgres -d store_db
...
2. Or with `docker run`:
   ```bash
   docker run -p 8000:4019 digitalvista:latest
   ```
...
```bash
docker exec digitalvista_postgres psql -U postgres -d store_db -c "\dt"
```
...
```bash
docker rmi digitalvista:latest
```
